...
BEGIN
  IF p_direction IN ('ancestors', 'both') THEN
    ...
    WITH RECURSIVE ancestors AS (
      ...
      LEFT JOIN platform.school_lineage sl ON sl.school_id = s.id
      ...
      FROM ancestors a
      JOIN platform.school_lineage sl2 ON sl2.school_id = a.parent_id
  ...
  IF p_direction IN ('descendants', 'both') THEN
    ...
    WITH RECURSIVE descendants AS (
      ...
      JOIN platform.martial_schools s ON s.id = sl2.school_id AND s.is_deleted = false
