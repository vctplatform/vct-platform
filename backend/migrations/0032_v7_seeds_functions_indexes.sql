...
-- authorization_tuples (CRITICAL for ReBAC performance)
...
CREATE INDEX IF NOT EXISTS idx_signatures_chain ON core.digital_signatures(previous_signature_id)
...
CREATE INDEX IF NOT EXISTS idx_notif_delivery_pending ON system.notification_deliveries(status, created_at)
    WHERE status IN ('PENDING', 'SENT');

-- notification_templates
CREATE INDEX IF NOT EXISTS idx_notif_templates_active ON system.notification_templates(category, channel, locale)
...
CREATE INDEX IF NOT EXISTS idx_gen_schedules_published ON tournament.generated_schedules(status)
    WHERE status = 'PUBLISHED';

-- document_templates
CREATE INDEX IF NOT EXISTS idx_doc_templates_type ON platform.document_templates(template_type)
    WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_integrity_alerts_severity ON tournament.integrity_alerts(severity)
    WHERE status NOT IN ('CLOSED', 'UNSUBSTANTIATED');
CREATE INDEX IF NOT EXISTS idx_integrity_alerts_assigned ON tournament.integrity_alerts(assigned_to)
    WHERE assigned_to IS NOT NULL AND status IN ('UNDER_REVIEW', 'INVESTIGATING');
...
-- federation_merges
CREATE INDEX IF NOT EXISTS idx_federation_merges_status ON core.federation_merges(status)
    WHERE status NOT IN ('COMPLETED', 'ROLLED_BACK');

-- sport_profiles
CREATE INDEX IF NOT EXISTS idx_sport_profiles_code ON platform.sport_profiles(sport_code)
    WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_daily_loads_status ON tournament.athlete_daily_loads(load_status)
    WHERE load_status IN ('HIGH', 'EXCESSIVE', 'BLOCKED');
