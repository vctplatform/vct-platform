...
export const VCT_EmptyState = ({
    ...
    const actionEl = action ?? (actionLabel && onAction ? (
        <button onClick={onAction} className="mt-1 inline-flex items-center gap-2 rounded-xl bg-(--vct-accent-cyan) px-5 py-2.5 text-sm font-bold text-(--vct-bg-base) shadow-[0_0_16px_var(--vct-accent-cyan)/30] transition-all hover:brightness-110 active:scale-[.97]">
            {actionLabel}
        </button>
    ) : null)
    ...
    return (
        <div className={`flex flex-col items-center justify-center text-center ${compact ? 'py-8' : 'py-16'} ${className}`}>
            <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-3xl bg-vct-input text-4xl shadow-(--vct-shadow-sm)">
                {icon}
            </div>
