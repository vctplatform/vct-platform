...
export const Page_provincial_sub_associations = () => {
    ...
    return (
        <VCT_PageContainer size="wide" animated>
            ...
            <div className="mb-6">
                <h1 className="text-2xl font-bold tracking-tight text-[var(--vct-text-primary)]">📌 Quản Lý Chi Hội Phường/Xã</h1>
                <p className="text-sm text-[var(--vct-text-secondary)] mt-1">Danh sách Chi hội Võ cổ truyền cấp phường/xã trực thuộc Hội quận/huyện</p>
            </div>

            <VCT_StatRow items={[
                { label: 'Tổng Chi hội', value: subAssociations.length, icon: <VCT_Icons.MapPin size={18} />, color: '#0ea5e9' },
                { label: 'Chi hội hoạt động', value: subAssociations.filter(s => s.status === 'active').length, icon: <VCT_Icons.Activity size={18} />, color: '#10b981' },
                { label: 'Chờ duyệt', value: subAssociations.filter(s => s.status === 'pending').length, icon: <VCT_Icons.Clock size={18} />, color: '#f59e0b' },
                { label: 'Tổng CLB', value: totalClubs, icon: <VCT_Icons.Building2 size={18} />, color: '#8b5cf6' },
                { label: 'Tổng VĐV', value: totalAthletes, icon: <VCT_Icons.Users size={18} />, color: '#ef4444' },
                    ...
                    <table className="w-full border-collapse">
                        ...
                        <tbody>
                            {filtered.map(sa => {
                                ...
                                return (
                                    <tr key={sa.id} className="border-b border-[var(--vct-border-subtle)] hover:bg-[var(--vct-bg-hover)] transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-sm">
                                            <span className="text-xs px-2 py-1 rounded-full bg-[var(--vct-bg-elevated)] text-[var(--vct-text-secondary)]">
                                                {sa.association_name}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-sm">{sa.leader_name}</td>
                                        <td className="px-4 py-3 text-sm text-center font-bold" style={{ color: '#22d3ee' }}>{sa.total_clubs}</td>
                                        <td className="px-4 py-3 text-sm text-center font-bold" style={{ color: '#8b5cf6' }}>{sa.total_athletes}</td>
