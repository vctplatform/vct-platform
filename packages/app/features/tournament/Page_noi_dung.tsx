...
import { NOI_DUNG_QUYENS, HANG_CANS, LUA_TUOIS, genId } from '../data/mock-data';
...
export const Page_noi_dung = () => {
    ...
    const [dataQuyen, setDataQuyen] = useState<NoiDungQuyen[]>([...NOI_DUNG_QUYENS]);
    const [dataDK, setDataDK] = useState<HangCan[]>([...HANG_CANS]);
    const [dataLT, setDataLT] = useState<LuaTuoi[]>([...LUA_TUOIS]);
    ...
    // ── CRUD QUYỀN ──
    const openAddQ = () => { setEditMode(false); setFormQ({ ...BLANK_Q, lua_tuoi: luaTuoiOptions[0]?.value || '' }); setShowModalQ(true); };
    const openEditQ = (item: NoiDungQuyen) => { setEditMode(true); setFormQ(item); setShowModalQ(true); };
    const handleSaveQ = () => {
    ...
    // ── CRUD ĐỐI KHÁNG ──
    const openAddDK = () => { setEditMode(false); setFormDK({ ...BLANK_DK, lua_tuoi: luaTuoiOptions[0]?.value || '' }); setShowModalDK(true); };
    const openEditDK = (item: HangCan) => { setEditMode(true); setFormDK(item); setShowModalDK(true); };
    const handleSaveDK = () => {
    ...
    // ── CRUD LỨA TUỔI ──
    const openAddLT = () => { setEditMode(false); setFormLT({ ...BLANK_LT }); setShowModalLT(true); };
    const openEditLT = (item: LuaTuoi) => { setEditMode(true); setFormLT(item); setShowModalLT(true); };
    const handleSaveLT = () => {
        ...
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <VCT_Stack direction="row" justify="space-between" align="center" style={{ marginBottom: 16 }}>
                    ...
                    <VCT_Button icon={<VCT_Icons.Plus size={16} />} onClick={openAddQ}>Thêm bài quyền</VCT_Button>
                </VCT_Stack>
                {filtered.length === 0 ? <VCT_EmptyState title="Không có dữ liệu" icon="🥋" /> : (
                    <VCT_Table data={filtered} columns={[
                        { key: 'ten', label: 'Tên bài quyền', render: (r: any) => <VCT_Text style={{ fontWeight: 700 }}>{r.ten}</VCT_Text> },
                        ...
                        { key: 'trang_thai', label: 'Trạng thái', render: (r: any) => <VCT_Badge text={ST_MAP[r.trang_thai].l} type={ST_MAP[r.trang_thai].t} /> },
                        {
                            key: 'actions', label: '', align: 'right', render: (r: any) => (
                                <VCT_Stack direction="row" gap={8} justify="flex-end">
                                    <button onClick={() => openEditQ(r)} style={{ background: 'none', border: 'none', color: 'var(--vct-text-tertiary)', cursor: 'pointer' }}><VCT_Icons.Edit size={16} /></button>
                                    <button onClick={() => setDeleteTarget({ type: 'quyen', id: r.id, name: r.ten })} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}><VCT_Icons.Trash size={16} /></button>
                                </VCT_Stack>
                            )
                        }
                    ]} />
        ...
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <VCT_Stack direction="row" justify="space-between" align="center" style={{ marginBottom: 16 }}>
                    ...
                    <VCT_Button icon={<VCT_Icons.Plus size={16} />} onClick={openAddDK}>Thêm hạng cân</VCT_Button>
                </VCT_Stack>
                {filtered.length === 0 ? <VCT_EmptyState title="Không có dữ liệu" icon="🥊" /> : (
                    <VCT_Table data={filtered} columns={[
                        { key: 'hang_can', label: 'Hạng cân', render: (r: any) => <VCT_Text style={{ fontWeight: 700 }}>{r.can_den ? `${r.can_tu} - ${r.can_den} kg` : `Trên ${r.can_tu} kg`}</VCT_Text> },
                        ...
                        { key: 'trang_thai', label: 'Trạng thái', render: (r: any) => <VCT_Badge text={ST_MAP[r.trang_thai].l} type={ST_MAP[r.trang_thai].t} /> },
                        {
                            key: 'actions', label: '', align: 'right', render: (r: any) => (
                                <VCT_Stack direction="row" gap={8} justify="flex-end">
                                    <button onClick={() => openEditDK(r)} style={{ background: 'none', border: 'none', color: 'var(--vct-text-tertiary)', cursor: 'pointer' }}><VCT_Icons.Edit size={16} /></button>
                                    <button onClick={() => setDeleteTarget({ type: 'doikhang', id: r.id, name: r.can_den ? `${r.can_tu}-${r.can_den}kg` : `>${r.can_tu}kg` })} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}><VCT_Icons.Trash size={16} /></button>
                                </VCT_Stack>
                            )
                        }
                    ]} />
        ...
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <VCT_Stack direction="row" justify="space-between" align="center" style={{ marginBottom: 16 }}>
                    ...
                    <VCT_Button icon={<VCT_Icons.Plus size={16} />} onClick={openAddLT}>Thêm lứa tuổi</VCT_Button>
                </VCT_Stack>
                {dataLT.length === 0 ? <VCT_EmptyState title="Không có dữ liệu" icon="📅" /> : (
                    <VCT_Table data={dataLT} columns={[
                        ...
                        { key: 'ten', label: 'Tên lứa tuổi', render: (r: any) => <VCT_Text style={{ fontWeight: 700 }}>{r.ten}</VCT_Text> },
                        { key: 'khoang', label: 'Khoảng tuổi', render: (r: any) => `${r.tu_tuoi} → ${r.den_tuoi} tuổi` },
                        { key: 'trang_thai', label: 'Trạng thái', render: (r: any) => <VCT_Badge text={ST_MAP[r.trang_thai].l} type={ST_MAP[r.trang_thai].t} /> },
                        {
                            key: 'actions', label: '', align: 'right', render: (r: any) => (
                                <VCT_Stack direction="row" gap={8} justify="flex-end">
                                    <button onClick={() => openEditLT(r)} style={{ background: 'none', border: 'none', color: 'var(--vct-text-tertiary)', cursor: 'pointer' }}><VCT_Icons.Edit size={16} /></button>
                                    <button onClick={() => setDeleteTarget({ type: 'luatuoi', id: r.id, name: r.ten })} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}><VCT_Icons.Trash size={16} /></button>
                                </VCT_Stack>
                            )
                        }
                    ]} />
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', paddingBottom: '100px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(200px, 1fr) minmax(280px, 1.5fr) minmax(280px, 1.5fr)', gap: '16px', marginBottom: '24px' }}>
                ...
                <VCT_Card>
                    <VCT_Stack direction="row" align="center" gap={12} style={{ marginBottom: 16 }}>
            ...
            <VCT_Modal isOpen={showModalQ} onClose={() => setShowModalQ(false)} title={`${editMode ? 'Sửa' : 'Thêm'} Bài Quyền`} width="600px" footer={
                <><VCT_Button variant="secondary" onClick={() => setShowModalQ(false)}>Hủy</VCT_Button><VCT_Button onClick={handleSaveQ}>Lưu</VCT_Button></>
            }>
                <VCT_Stack gap={16}>
                    ...
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Đội hình thi đấu" style={{ flex: 1 }}><VCT_Select options={[{ value: 'ca_nhan', label: 'Cá nhân' }, { value: 'doi', label: 'Đôi' }, { value: 'dong_doi', label: 'Đồng đội' }]} value={formQ.hinh_thuc} onChange={(v: any) => setFormQ({ ...formQ, hinh_thuc: v })} /></VCT_Field>
                        <VCT_Field label="Hình thức thi" style={{ flex: 1 }}><VCT_Select options={[{ value: 'theo_diem', label: 'Tính điểm xếp hạng' }, { value: 'dau_loai_ban_ket', label: 'Đấu loại từ Bán kết (Top 4)' }]} value={formQ.hinh_thuc_thi_dau || 'theo_diem'} onChange={(v: any) => setFormQ({ ...formQ, hinh_thuc_thi_dau: v })} /></VCT_Field>
                    </VCT_Stack>
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Giới tính" style={{ flex: 1 }}><VCT_Select options={[{ value: 'nam', label: 'Nam' }, { value: 'nu', label: 'Nữ' }, { value: 'nam_nu', label: 'Nam Nữ' }]} value={formQ.gioi} onChange={(v: any) => setFormQ({ ...formQ, gioi: v })} /></VCT_Field>
                        <VCT_Field label="Lứa tuổi" style={{ flex: 1 }}><VCT_Select options={luaTuoiOptions} value={formQ.lua_tuoi} onChange={(v: any) => setFormQ({ ...formQ, lua_tuoi: v })} /></VCT_Field>
                    </VCT_Stack>
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Trạng thái" style={{ flex: 1 }}><VCT_Select options={[{ value: 'active', label: 'Hoạt động' }, { value: 'draft', label: 'Nháp' }, { value: 'closed', label: 'Đóng' }]} value={formQ.trang_thai} onChange={(v: any) => setFormQ({ ...formQ, trang_thai: v })} /></VCT_Field>
                    </VCT_Stack>
            ...
            <VCT_Modal isOpen={showModalDK} onClose={() => setShowModalDK(false)} title={`${editMode ? 'Sửa' : 'Thêm'} Hạng Cân Đối Kháng`} width="600px" footer={
                <><VCT_Button variant="secondary" onClick={() => setShowModalDK(false)}>Hủy</VCT_Button><VCT_Button onClick={handleSaveDK}>Lưu</VCT_Button></>
            }>
                <VCT_Stack gap={16}>
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Cân từ (kg) *" style={{ flex: 1 }}><VCT_Input type="number" value={formDK.can_tu} onChange={(e: any) => setFormDK({ ...formDK, can_tu: Number(e.target.value) })} /></VCT_Field>
                        <VCT_Field label="Cân đến (kg) (0 = vô hạn) *" style={{ flex: 1 }}><VCT_Input type="number" value={formDK.can_den} onChange={(e: any) => setFormDK({ ...formDK, can_den: Number(e.target.value) })} /></VCT_Field>
                    </VCT_Stack>
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Giới tính" style={{ flex: 1 }}><VCT_Select options={[{ value: 'nam', label: 'Nam' }, { value: 'nu', label: 'Nữ' }]} value={formDK.gioi} onChange={(v: any) => setFormDK({ ...formDK, gioi: v })} /></VCT_Field>
                        <VCT_Field label="Lứa tuổi" style={{ flex: 1 }}><VCT_Select options={luaTuoiOptions} value={formDK.lua_tuoi} onChange={(v: any) => setFormDK({ ...formDK, lua_tuoi: v })} /></VCT_Field>
                    </VCT_Stack>
            ...
            <VCT_Modal isOpen={showModalLT} onClose={() => setShowModalLT(false)} title={`${editMode ? 'Sửa' : 'Thêm'} Nhóm Tuổi`} width="600px" footer={
                <><VCT_Button variant="secondary" onClick={() => setShowModalLT(false)}>Hủy</VCT_Button><VCT_Button onClick={handleSaveLT}>Lưu</VCT_Button></>
            }>
                <VCT_Stack gap={16}>
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Mã quy ước *" style={{ flex: 1 }}><VCT_Input value={formLT.ma} onChange={(e: any) => setFormLT({ ...formLT, ma: e.target.value })} placeholder="VD: thieu_nien" /></VCT_Field>
                        <VCT_Field label="Tên hiển thị *" style={{ flex: 1 }}><VCT_Input value={formLT.ten} onChange={(e: any) => setFormLT({ ...formLT, ten: e.target.value })} placeholder="VD: Thiếu niên" /></VCT_Field>
                    </VCT_Stack>
                    <VCT_Stack direction="row" gap={16}>
                        <VCT_Field label="Từ (tuổi) *" style={{ flex: 1 }}><VCT_Input type="number" value={formLT.tu_tuoi} onChange={(e: any) => setFormLT({ ...formLT, tu_tuoi: Number(e.target.value) })} /></VCT_Field>
                        <VCT_Field label="Đến (tuổi) *" style={{ flex: 1 }}><VCT_Input type="number" value={formLT.den_tuoi} onChange={(e: any) => setFormLT({ ...formLT, den_tuoi: Number(e.target.value) })} /></VCT_Field>
                    </VCT_Stack>
